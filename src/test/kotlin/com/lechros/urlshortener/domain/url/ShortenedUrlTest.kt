...
class ShortenedUrlTest : StringSpec({
    "단축 URL 생성" {
        ...
        val url = ShortenedUrl("target-url", "short-path", now)
        assertSoftly(url) {
            targetUrl shouldBe "target-url"
            shortPath shouldBe "short-path"
    ...
    "만료 일자가 지나지 않은 URL은 만료 여부가 거짓이다." {
        ...
        val url = ShortenedUrl("target-url", "short-path", now, expiresAt)
